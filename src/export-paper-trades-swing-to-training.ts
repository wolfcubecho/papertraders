#!/usr/bin/env node
/**
 * Export SWING paper trades to ML training CSV.
 *
 * Reads saved features directly from trade records (no Binance API calls needed).
 * Trades without saved features are skipped (old trades before feature saving).
 *
 * Usage:
 *   npm run export-paper-trades-swing
 */
...
// A trade from the swing trader JSON files
...
function groupClosedTrades(allTrades: SavedTrade[], intervalMs: number): GroupedTrade[] {
  ...
  for (const [, trades] of groups.entries()) {
    ...
    result.push({
      ...
      entryFeatures: first.entryFeatures,
      exitFeatures: last.exitFeatures,
      regime: first.regime,
...
async function main(): Promise<void> {
  ...
  const outCsv = path.join(outputDir, `paper_swing_${timestamp}.csv`);
  const outJson = path.join(outputDir, `paper_swing_${timestamp}.json`);
